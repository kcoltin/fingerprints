...
# Kevin Coltin
# 
# This is a script, modified from the file mexopts.sh that ships with MATLAB,
# for compiling the fingerprints module. 
...
#
            FC='gfortran'
            FFLAGS='-fexceptions -fbackslash --free-form -Wall -Wuninitialized -Wtabs -fbounds-check -fbacktrace -largeArrayDims' # Kevin added free form, warnings, bounds checking, backtrace, and large array dims
            FFLAGS="$FFLAGS -fPIC -fno-omit-frame-pointer"
            FLIBS="$RPATH $MLIBS -lm"
            FOPTIMFLAGS='-O2' # Changed by Kevin; was O 
            FDEBUGFLAGS='-g'
#
            LD="$COMPILER"
            LDEXTENSION='.mexa64'
            LDFLAGS="-pthread -shared -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE -Wl,--no-undefined"
            LDOPTIMFLAGS='-O3' # Changed by Kevin; was O 
            LDDEBUGFLAGS='-g'
#
            POSTLINK_CMDS=':'
#----------------------------------------------------------------------------
